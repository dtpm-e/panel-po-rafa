...
def param_faltantes_A3(A3_direccion, ruta_OUTPUT):
    ...
    Datos_con_ceros = A3[
        (A3["N° SALIDAS"] > 0) &
        ((A3["VELOCIDAD (Km/hra)"] <= 0) | (A3["DISTANCIA BASE (Km)"] <= 0) |
         (A3["DISTANCIA TOTAL (POB+POI) (Km)"] <= 0) | (A3["CAPACIDAD (plazas)"] <= 0))
    ]
...
def revisar_horarios_a1_a4(A1_direccion, A4_direccion, ruta_OUTPUT):
    ...
    A1_tramos_horarios = pd.merge(A1_tramos_horarios, A1_tramos_horarios, on=['CODIGO TS SERVICIO',
                                                                              'SENTIDO', 'TIPO DIA'])
    A1_tramos_horarios = A1_tramos_horarios[
        A1_tramos_horarios['TRAMO HORARIO_x'] != A1_tramos_horarios['TRAMO HORARIO_y']]
    A1_tramos_horarios = A1_tramos_horarios[
        A1_tramos_horarios.duplicated(['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA'])]
    ...
    ifh["¿Respeta horarios?"] = ifh.apply(lambda row: isNowInTimePeriod(row['HORA INICIO'], row['HORA TERMINO'],
                                                                        row['HORA_INICIO']), axis=1)
    ...
    a4a1_tramos_horarios = pd.merge(a4th_com, A1_tramos_horarios, on=['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA'],
                                    suffixes=('_a4','_a1_tramos_horario'))
    ifh_tramos_horarios = a4a1_tramos_horarios[['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA', "HORA INICIO_x",
                                                "HORA TERMINO_x", "HORA INICIO_y", "HORA TERMINO_y", "HORA_INICIO"]]
    if len(ifh_tramos_horarios) > 0:
        ifh_tramos_horarios["¿Respeta horario_x?"] = ifh_tramos_horarios.apply(lambda row: isNowInTimePeriod(
            row['HORA INICIO_x'], row['HORA TERMINO_x'], row['HORA_INICIO']), axis=1)
        ifh_tramos_horarios["¿Respeta horario_y?"] = ifh_tramos_horarios.apply(lambda row: isNowInTimePeriod(
            row['HORA INICIO_y'], row['HORA TERMINO_y'], row['HORA_INICIO']), axis=1)
        ifh_tramos_horarios["¿Respeta alguno de sus horarios?"] = ifh_tramos_horarios.apply(
            lambda row: row["¿Respeta horario_x?"] | row["¿Respeta horario_y?"], axis=1)
        ifh_tramos_horarios = ifh_tramos_horarios[['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA', "HORA INICIO_x",
                                                   "HORA TERMINO_x", "HORA INICIO_y", "HORA TERMINO_y",
                                                   "HORA_INICIO", "¿Respeta alguno de sus horarios?"]]
    else:
    ...
    cambios_nombre_output = {'HORA_INICIO': 'HORA EXPEDICION (A4)',
                             'HORA INICIO_x': 'Horario 1 inicio servicio (A1)',
                             'HORA INICIO_y': 'Horario 2 inicio servicio (A1)',
                             'HORA TERMINO_x': 'Horario 1 termino servicio (A1)',
                             'HORA TERMINO_y': 'Horario 2 termino servicio (A1)',
                             'HORA INICIO': 'Horario inicio servicio (A1)',
                             'HORA TERMINO': 'Horario termino servicio (A1)'}
    ...
    for i, row in A1.iterrows():
        ...
        hi = row['HORA INICIO']
        ht = row['HORA TERMINO']
        expedicion_hora_inicio = A4th[(A4th['CODIGO TS SERVICIO'] == servicio) & (A4th['SENTIDO'] == sentido) &
                                      (A4th['TIPO DIA'] == td) & (A4th['HORA_INICIO'] == hi)]
        expedicion_hora_termino = A4th[(A4th['CODIGO TS SERVICIO'] == servicio) & (A4th['SENTIDO'] == sentido) &
                                       (A4th['TIPO DIA'] == td) & (A4th['HORA_INICIO'] == ht)]
        if len(expedicion_hora_inicio.axes[0]) == 0:
            sin_hi = pd.DataFrame({'CODIGO TS SERVICIO': [row['CODIGO TS SERVICIO']],
                                   'SENTIDO': [row['SENTIDO']],
                                   'TIPO DIA': [row['TIPO DIA']],
                                   'No hay expedición en esta Hora Inicio': [hi]})
            hini_sin_salida.append(sin_hi)
        if len(expedicion_hora_termino.axes[0]) == 0:
            sin_ht = pd.DataFrame({'CODIGO TS SERVICIO': [row['CODIGO TS SERVICIO']],
                                   'SENTIDO': [row['SENTIDO']],
                                   'TIPO DIA': [row['TIPO DIA']],
                                   'No hay expedición en esta Hora Termino': [ht]})
...
def contar_salidas(A3_direccion,A4_direccion,ruta_OUTPUT):
    ...
    comparacionSalidasXMH = pd.merge(salidas_xmh_a3, salidas_xmh_th, on=['CODIGO TS SERVICIO', 'SENTIDO',
                                                                         'TIPO DIA', 'MH'],
                                     suffixes=('_A3','_TH'), how='outer', indicator=True)
    salidas_sin_th_xmh = comparacionSalidasXMH[(comparacionSalidasXMH['_merge'] == 'left_only') &
                                               (comparacionSalidasXMH['N° SALIDAS_A3'] > 0)]
    ...
    comparacionSalidasXMH["Diferencia entre salidas"] = comparacionSalidasXMH.apply(lambda row:
                                                                                    row['N° SALIDAS_A3'] -
                                                                                    row['N° SALIDAS_TH'], axis=1)


    comparacionSalidasXMHA = comparacionSalidasXMH[['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA', 'AGRUPACIÓN MH',
                                                    'N° SALIDAS_A3', 'N° SALIDAS_TH']]
    comparacionSalidasXMHA = comparacionSalidasXMHA.groupby(['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA',
                                                             'AGRUPACIÓN MH']).sum().reset_index()
    comparacionSalidasXMHA["Diferencia entre salidas"] = comparacionSalidasXMHA.apply(lambda row: row['N° SALIDAS_A3'] -
                                                                                                  row['N° SALIDAS_TH'],
                                                                                      axis=1)
    ...
    ###Redundante
    dif_salidas_XMHA, dif_salidas_XMH = Datos_dif_salidas_XMHA, Datos_dif_salidas_XMH
    dif_salidas_sin_th_xmh, dif_salidas_sin_a3_xmh = salidas_sin_th_xmh, salidas_sin_a3_xmh
...
def calcular_capacidades(A3_direccion,A4_direccion,ruta_OUTPUT):
    ...
    capacidad_xmh_th = a4th.groupby(['CODIGO TS SERVICIO', 'SENTIDO',
                                     'TIPO DIA', 'PERIODO_INICIO'])['Capacidad'].sum().reset_index() #Sumamos capacidad
    capacidad_xmh_th = capacidad_xmh_th.rename(columns={'PERIODO_INICIO': 'MH',
                                                        'TIPO_DIA': 'TIPO DIA',
                                                        'Capacidad': 'CAPACIDAD (plazas)'})

    comparacionCapacidadXMH = pd.merge(capacidad_xmh_a3, capacidad_xmh_th, on=['CODIGO TS SERVICIO', 'SENTIDO',
                                                                               'TIPO DIA', 'MH'],suffixes=('_A3','_TH'))
    comparacionCapacidadXMH["Diferencia entre Capacidad [Plazas]"] = comparacionCapacidadXMH.apply(
        lambda row: row['CAPACIDAD (plazas)_A3'] - row['CAPACIDAD (plazas)_TH'], axis=1)

    comparacionCapacidadXMHA = comparacionCapacidadXMH[['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA', 'AGRUPACIÓN MH',
                                                        'CAPACIDAD (plazas)_A3', 'CAPACIDAD (plazas)_TH']]
    comparacionCapacidadXMHA['CAPACIDAD (plazas)_TH'] = comparacionCapacidadXMHA[['CAPACIDAD (plazas)_TH'
                                                                                  ]].apply(pd.to_numeric)

    comparacionCapacidadXMHA = comparacionCapacidadXMHA.groupby(['CODIGO TS SERVICIO',
                                                                 'SENTIDO',
                                                                 'TIPO DIA',
                                                                 'AGRUPACIÓN MH']
                                                                )['CAPACIDAD (plazas)_A3',
                                                                  'CAPACIDAD (plazas)_TH'].sum().reset_index()

    comparacionCapacidadXMHA["Diferencia entre Capacidad [Plazas]"] = comparacionCapacidadXMHA.apply(
        lambda row: row['CAPACIDAD (plazas)_A3'] - row['CAPACIDAD (plazas)_TH'], axis=1)
    Datos_dif_capacidad_XMHA = comparacionCapacidadXMHA[comparacionCapacidadXMHA[
                                                            "Diferencia entre Capacidad [Plazas]"] != 0]
    Datos_dif_capacidad_XMH = comparacionCapacidadXMH[comparacionCapacidadXMH[
                                                          "Diferencia entre Capacidad [Plazas]"] != 0]
    #Falta entregar un archivo con los sstd con
    ##Moverse a salida
    os.chdir(ruta_OUTPUT)
...
def revisar_dist(A3_direccion, A4_direccion, ruta_OUTPUT):
    ...
    A4TH = A4TH.rename(columns={'PERIODO_INICIO': 'MH',
                                'TIPO_DIA': 'TIPO DIA',
                                'Capacidad': 'CAPACIDAD (plazas)'})
    ##Union de ambos anexos
    A4THcdist = pd.merge(A3, A4TH, on=['CODIGO TS SERVICIO', 'SENTIDO', 'TIPO DIA', 'MH'],
                         suffixes=('_A3','_TH'), how='right')
    #Se calcula la resta de la distancia reportada en A3 con la del A4
    A4THcdist["Diferencia entre Distancia [Km]"] = A4THcdist.apply(
        lambda row: row['DISTANCIA TOTAL (POB+POI) (Km)'] - row['DISTANCIA'], axis=1)
    dif_dist = A4THcdist[A4THcdist["Diferencia entre Distancia [Km]"] != 0]
    dif_dist = dif_dist[['CODIGO TS SERVICIO', 'CODIGO USUARIO SERVICIO', 'SENTIDO', 'TIPO DIA', 'MH', 'HORA_INICIO',
                         'DISTANCIA TOTAL (POB+POI) (Km)', 'DISTANCIA', 'Diferencia entre Distancia [Km]']]
    dif_dist = dif_dist.rename(columns={'DISTANCIA TOTAL (POB+POI) (Km)': 'DISTANCIA TOTAL A3',
                                        'DISTANCIA': 'DISTANCIA TOTAL A4'})
...
def revisar_intervalos(A1_direccion, A3_direccion, A4_direccion, ruta_OUTPUT):
    ...
    A1['HORARIO INICIO'] = A1.apply(lambda row: dt.datetime(1995, 6, 13, row['HORA INICIO'].hour,
                                                            row['HORA INICIO'].minute, row['HORA INICIO'].second),
                                    axis=1)
    A1['HORARIO TERMINO'] = A1.apply(lambda row: convertir_horario_dia_siguiente(row['HORA INICIO'],
    ...
    a4tha1['FH IE'] = a4tha1.apply(lambda row: convertir_horasalida_nextday(row['HORA_INICIO'].hour,
                                                                            row['HORA_INICIO'].minute,
                                                                            row['HORA_INICIO'].second,
                                                                            row['HORARIO INICIO'],
                                                                            row['HORARIO TERMINO']), axis=1)
    ...
    for servicio in Servicios:
        for sentido in Sentidos:
            for td in Tipos_de_dias:
                ...
                th_sstd = a4thMHA[(a4thMHA['CODIGO TS SERVICIO'] == servicio) &
                                  (a4thMHA['SENTIDO'] == sentido) &
                                  (a4thMHA['TIPO DIA'] == td)]
                    ...
                    ###Iteramos sobre cada MHA
                        for PerAgr in range(1, max_agrupacion+1):  #Periodos Agrupados = PerAgr
                            ...
                            for i, row in th_sstdMHA.iterrows():
                                ...
                                res_x_exp = pd.DataFrame({'CODIGO TS SERVICIO': [row['CODIGO TS SERVICIO']],
                                                          'SENTIDO': [row['SENTIDO']],
                                                          'TIPO DIA': [row['TIPO DIA']],
                                                          'AGRUPACIÓN MH': [row['AGRUPACIÓN MH']],
                                                          'Hora inicio exp': [row['FH IE']],
                                                          'Cumplimiento Intervalo ManualPPO': [resultado],
                                                          'Intervalo prom MHA': [intervalo_promedio]})
    ...
    for servicio in Servicios:
        for sentido in Sentidos:
            ...
            ##Debo agarrar la tabla horaria solo de ese servicio
                th_sstd = a4thMHA[(a4thMHA['CODIGO TS SERVICIO'] == servicio) &
                                  (a4thMHA['SENTIDO'] == sentido) &
                                  (a4thMHA['TIPO DIA'] == td)].reset_index(drop=True)
                if len(th_sstd.axes[0]) == 0:
                    pass
                else:
    ...
    for servicio in Servicios:
        for sentido in Sentidos:
            for td in Tipos_de_dias:
                I_sstd = IpromMH[(IpromMH['CODIGO TS SERVICIO'] == servicio) &
                                 (IpromMH['SENTIDO'] == sentido) &
                                 (IpromMH['TIPO DIA'] == td)].reset_index(drop=True)
...
#Termina despues de las 0 horas y la expedicion empieza despues de las 00 horas y esas expediciones son antes de la hora de inicio si no fuera el contexto de un reloj
    if (horario_fin.time() >= dt.time(hour=0
                                      )) and dt.time(hora,
                                                     minutos,
                                                     segundos) >= dt.time(hour=0
                                                                          ) and dt.time(hora,
                                                                                        minutos,
...
def convertir_horario_dia_siguiente(HI, HT):
    ...
    if HI.__class__.__name__ == 'time' and HT.__class__.__name__ == 'time':
        if HI < HT:
            ...
            return HT_ARREG
        elif HI > HT >= dt.time(0, 0): #Condicion que implica que las expediciones siguientes pasan al otro dia
