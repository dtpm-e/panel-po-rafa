...
,'TIPO_BUS':'Tipologia'\
,'PERIODO_INICIO':'MH','TIPO_DIA':'TIPO DIA','DISTANCIA (KM)':'DISTANCIA'}
    
##Diccionario que corrige tipico error en columnas del A4  TH 
dicc_a4th={'TIPO DIA':'TIPO_DIA','DISTANCIA (KM)':'DISTANCIA','DISTANCIA (km)':'DISTANCIA'}
...
def formatear_a3(A3_dir):
    ...
    A3_sf['DISTANCIA TOTAL (POB+POI) (Km)']=A3_sf.apply(lambda row: detectar_decimales(row['DISTANCIA TOTAL (POB+POI) (Km)']),axis=1)
...
def formatear_a1(A1_dir):
    ...
    A1_sf.rename(columns={'CÓDIGO Usuario servicio':'CODIGO USUARIO SERVICIO','Sentido':'SENTIDO','TIPO_DIA':'TIPO DIA'},inplace=True)
    ...
    A1_sf['HORA INICIO']=A1_sf.apply(lambda row: dt.datetime(1995,6,13,row['HORA INICIO'].hour,row['HORA INICIO'].minute,row['HORA INICIO'].second),axis=1)
    A1_sf['HORA TERMINO']=A1_sf.apply(lambda row: fix_string_as_time(row['HORA TERMINO']),axis=1)
    A1_sf['HORA TERMINO']=A1_sf.apply(lambda row: convertir_horario_dia_siguiente_func(row['HORA INICIO'],row['HORA TERMINO']),axis=1)
...
def formatear_a1_vRH(A1_dir):
    ...
    A1_sf['TRAMO HORARIO']=A1_sf.apply(lambda row: int(row['TRAMO HORARIO']),axis=1)
    A1_sf['HORA INICIO']=A1_sf.apply(lambda row: fix_string_as_time(row['HORA INICIO']),axis=1)
    A1_sf['HORA INICIO']=A1_sf.apply(lambda row: dt.datetime(1995,6,13,row['HORA INICIO'].hour,row['HORA INICIO'].minute,row['HORA INICIO'].second).time(),axis=1)
    A1_sf['HORA TERMINO']=A1_sf.apply(lambda row: fix_string_as_time(row['HORA TERMINO']),axis=1)
    A1_sf['HORA TERMINO']=A1_sf.apply(lambda row: convertir_horario_dia_siguiente_func(row['HORA INICIO'],row['HORA TERMINO']).time(),axis=1)
    #A1_sf['HORA TERMINO']=A1_sf.apply(lambda row: dt.datetime(1995,6,13,row['HORA TERMINO'].hour,row['HORA TERMINO'].minute,row['HORA TERMINO'].second).time(),axis=1)
    A1_formateado=A1_sf
    return A1_formateado
